...
# Lazy singletons — reused across warm invocations to avoid reconnection overhead
_fs_client = None
_bq_client = None
...
def process_room_completion(cloud_event):
    ...
    Extracts room_id from the event path, then reads status directly from Firestore.
    Skips processing if the room is not finished.
    """
    print("=== ANALYTICS WORKER CALLED ===")

    # Extract room_id from the document name in the event.
    # Eventarc always provides the document path even when data arrives as Protobuf bytes.
    room_id = None
    try:
    ...
    # Fallback: extract from CloudEvent attributes
    if not room_id:
        try:
    ...
    # Last resort: scan raw bytes for a rooms/{roomId} pattern
    if not room_id:
        try:
    ...
    if not room_id or room_id == "qhoot-database":
        print("Could not extract a valid room_id, skipping.")
        return

    # Read the current room status directly from Firestore
...
def _aggregate_and_stream(room_id: str, room_data: dict):
    ...
    # ── Per-question stats ────────────────────────────────────────────────────
    # Aggregate answers stored as answers.q0, answers.q1, ... on each player doc
    ...
    for player_data in players.values():
        ...
        for key, ans in answers.items():
            if not isinstance(ans, dict):
                continue
            try:
                q_index = int(key[1:])  # "q0" -> 0, "q1" -> 1, etc.
